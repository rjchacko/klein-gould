...
//				System.out.println("etak = " + etaK[i]);
	...
	*/
	private void findMatrix() {
		...
		for (int i = 0; i < Lp; i++){
			for (int j = 0; j <= i; j++){
				M[i][j]=M[j][i]=ising.mobility[i]*(-ising.T*f_k[(i-j+Lp)%Lp]/Lp);
			}
		}
		for (int i = 0; i < Lp; i++){
			if(i >= Lp/2)
			...
			if(ising.circleInteraction)
				M[i][i]-=ising.mobility[i]*ising.findVkCircle(Math.sqrt(kxValue*kxValue + kyValue*kyValue));
			else
				M[i][i]-=ising.mobility[i]*ising.findVkSquare(kxValue, kyValue);
		}
	}
