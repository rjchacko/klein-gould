...
//			// FOR DEBUGGING ONLY
	...
	public int evolveD(int mct, boolean takedata){
		...
		GR = 1; // the seed site
//		///////////
//		PrintUtil.printlnToFile(getOutdir()+File.separator+"debug2.txt","------ \n" + fs[newindex] + "\n------");
//		///////////
			...
			for (int jj = a ; jj < b ; jj++){
				...
				for(int kk = 0 ; kk < qN ; kk++){
					...
					if((stress[tmpnb] > sf[tmpnb])){
						...
						failSite(tmpnb);
//						///////////
//						PrintUtil.printlnToFile(getOutdir()+File.separator+"debug2.txt",tmpnb);
//						///////////
						// discharge forced site(s) and repeat until lattice is stable
					}
				}
	...
	public int getDorA(int site){
		
		return Lives[site] > 0 ? 1 : 0;
	}
