...
public class Metastablelifetime extends Simulation
{
	...
	public void Multicopies(IsingStructure ising, double T, double H, int copies, String dynamics)
	{
		...
		for(int copy=0; copy<copies; copy++)
		{
			...
			Lifetime(ising, T, H, totalruns, dynamics);
	...
	public void Lifetime(IsingStructure ising, double T, double H, int runs, String dynamics)
	{
		...
		for(int run=0; run< runs; run++)
		{
			...
			{
				Istemp.MCS(T, H, hrand, 1, dynamics);
				Job.animate();
			...
			params.set("H", -H);
			for(step=0; Istemp.Magnetization()>(threshold*Ms); step++)
			{
				
				Istemp.MCS(T, -H, srand, 1, dynamics);
				Job.animate();
				params.set("MCS", step);
			}
	...
	public void run(){
	    ...
	    //Scanfield(IS, T, 1.00, 0.70, 0.05, 200000, dynamics);
	    //Scanfield(IS, T, 0.70, 0.50, 0.02, 200000, dynamics);
	    //Scanfield(IS, T, 0.50, 0.40, 0.01, 200000, dynamics);
	    //Scanfield(IS, T, 0.40, 0.01, 0.005, 200000, dynamics);
