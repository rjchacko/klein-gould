...
public class NfailDamage2DV2 extends SimpleDamage2D{
	
	// Parameters (Private)
	 private double Sr0, Sc0, Srwidth, Scwidth, alphawidth, lifewidth, DaboutFS, tkip, dtkip;
	 private int Nlives, Nshowers, SonFSindex, NdeadS, Nalive, search, toKillindex;
	 private String lifeshape, residualnoise, criticalnoise, outdir, outfile1, PicDir, outfile2;
	 private double Sr[], Sc[], SsoFar[];
	 private double SonFS[], NlivesLeft[];
	...
	// Constructor
	public NfailDamage2DV2(Parameters params) {
	...
	public void Initialize(String str){
		...
		if(!(shape.equals("All Sites"))) fillNbrs();
	...
	private void fillNbrs(){
		
		if (BCs.equals("Periodic")){
			...
			// I should write this one of these day
		}
	...
	private void DistributeAndSearch(String subset){
		
		if(subset.equals("All Sites")){	// stress is being distributed to ALL of the lattice sites
				...
				if (Nalive > 0){
					...
					if(Nbool){
						for (int j = 0 ; j < dead[i] ; j++){
							...
							if (stress[j] > Sc[j]) toKill[toKillindex++]=j;
						}
		...
		else{	// stress is being distributed to a subset of the lattice sites
			...
			for (int i = 0; i<search; i++){
				int ds = dead[i];
				Nalive = 0;
				for (int j = 1 ; j <= NbrArray[ds][0] ; j++){
					Nalive += alive[NbrArray[ds][j]];
				}
				if(Nalive > 0){
					if(alive[ds]>0){
						release = (stress[ds] - Sr[ds])/Nalive;
					}
					else{
						release = stress[ds]/Nalive;
					}
				}
				if(Nbool){	// noise on alpha
					for (int j = 1 ; j <= NbrArray[ds][0] ; j++){
						stress[NbrArray[ds][j]] += alive[NbrArray[ds][j]]*release*(1 - alphawidth*rand.nextGaussian()-alpha);
						if(stress[NbrArray[ds][j]] > Sc[NbrArray[ds][j]]) toKill[toKillindex++] = NbrArray[ds][j];
					}
				}
				else{	// no noise on alpha
					for (int j = 1 ; j <= NbrArray[ds][0] ; j++){
						stress[NbrArray[ds][j]] += alive[NbrArray[ds][j]]*release*(1 - alpha);
						if(stress[NbrArray[ds][j]] > Sc[NbrArray[ds][j]]) toKill[toKillindex++] = NbrArray[ds][j];
					}
				}		
			}
	...
	private void setAvalanche(){
		
		search=0;
		
		for (int i = 0; i<toKillindex ; i++){
			int ks = toKill[i];
			SonFS[SonFSindex++]=stress[ks];
			dead[search++]=ks;
			alive[ks+N]=0;
			NlivesLeft[alive[ks]]--;
			if(alive[ks]>0) NlivesLeft[alive[ks]-1]++;
			alive[ks]--;
		}
		
		Nshowers+=toKillindex;
		
		return;
	}
	
//
//	private void DistStress(String HowToDumpStress){		
//		
...
//	}
	...
	/*
	 * 	THIS METHOD IS WORKING PROPERLY, HOWEVER, IT IS
	 * 	NOT BEING USED.
	 *
	 */

//	private int[] getStressLines(int center, int Nlines){
//		
...
//	}
	...
	private double radiusGyration(int index){
	...
	private double FSdensity(int index){
	...
	public int[] retNbs(int site){
		if(site < N){
			int[] _ret = new int[NbrArray[site][0]];
			for (int jj = 0 ; jj < NbrArray[site][0] ; jj++){
				_ret[jj] = NbrArray[site][jj+1];
			}
			return _ret;
		}
		else{
