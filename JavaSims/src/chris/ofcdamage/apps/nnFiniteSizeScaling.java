...
public class nnFiniteSizeScaling extends Simulation{
	...
	private int pt, ptmax, cycle, Lnow;
	private boolean draw;
	
	public static void main(String[] args) {
		new Control(new nnFiniteSizeScaling(), "OFC Parameters");
	}
	
public void load(Control c) {
		
		params.add("Data Directory",new DirectoryValue("/Users/cserino/Research/Data2/FractalDim/"));
		params.add("Random Seed",0);
		params.add("Interaction Shape", new ChoiceValue("Diamond","Circle","Square","All Sites"));
		params.add("Interaction Radius (R)",(int)(1));
		params.add("Lattice Size",10);
		...
		params.add("Number of Cycles", (int) 10);
	...
	public void run() {
		...
		Lnow  = params.iget("Lattice Size");
		
		
		while(Lnow < 1000){
			params.set("Lattice Size",Lnow);
			params.set("Equil Time",2*Lnow*Lnow);
	...
	private void configOF(){
		...
		Damage2D.PrintParams(model.getOutdir()+File.separator+"Params_"+100*Lnow+(cycle+1)+".txt",params);	
		model.setDataFile(100*Lnow+cycle+1);
...
//			}
	...
	private void recordClusters(){
		...
		int pcPN = model.pcnPC();
		int[] tmp = model.getClusters();
		int count = 0;
		
		for (int jj = 0 ; jj < model.intN() ; jj++){
			if (tmp[jj] == pcPN) count++;
		}
		
		
//		PrintUtil.printlnToFile(model.getOutdir()+File.separator+"Clusters_"+(100*Lnow+(cycle+1))+".txt", "Percolating Cluster Number", model.pcnPC());
//		PrintUtil.printArrayToFile(model.getOutdir()+File.separator+"Clusters_"+(100*Lnow+(cycle+1))+".txt",model.getClusters(),model.getL()*model.getL(),1);
//		model.printCdistance(100*Lnow+cycle);

		PrintUtil.printlnToFile(model.getOutdir()+File.separator+"masterDataFile2.txt",Lnow,model.getSize(model.pcnPC()),count,model.getNdead());
		return;
	}
