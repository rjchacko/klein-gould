...
public class CartesianInteractive extends CartesianType1 implements Selectable {
  ...
  public static final int INSIDE = 0, HORZ_MIN = 1, HORZ_MAX = 2, VERT_MIN = 3, VERT_MAX = 4, HORZ_AXIS = 5, HORZ_AXIS_MIN = 6, HORZ_AXIS_MAX = 7, VERT_AXIS = 8, VERT_AXIS_MIN = 9, VERT_AXIS_MAX = 10, HORZ_VAR = 11, VERT_VAR = 12;
  // instance fields
  ...
  public void draw(DrawingPanel panel, Graphics g) {
    ...
    if(!panel.isFixedScale()&&(scaleSetter!=null)&&scaleSetter.isVisible()&&(scaleSetter.scaleField.getBackground()!=Color.yellow)) {
  ...
  protected int findRegion(Point p) {
    ...
    if(hitRect.contains(p)) {
      ...
      scaleSetter.pack();
      Point fieldLoc = field.getLocation(); // relative to dialog
      scaleSetter.setLocation(plotLoc.x+hitLoc.x-fieldLoc.x, plotLoc.y+hitLoc.y-fieldLoc.y-offset);
      return HORZ_MIN;
    }
    // horizontal max
    ...
    if(hitRect.contains(p)) {
      ...
      scaleSetter.pack();
      Point fieldLoc = field.getLocation(); // relative to dialog
      int minLoc = plotLoc.x+hitLoc.x-fieldLoc.x;
      scaleSetter.setLocation(Math.max(minLoc, plotLoc.x+1-fieldLoc.x), plotLoc.y+hitLoc.y-fieldLoc.y);
      return VERT_MIN;
    }
    // vertical max
   ...
   */
  private ScaleSetter getScaleSetter() {
    if(scaleSetter==null) {
      boolean restore = JDialog.isDefaultLookAndFeelDecorated();
      JDialog.setDefaultLookAndFeelDecorated(false);
      scaleSetter = new ScaleSetter();
      JDialog.setDefaultLookAndFeelDecorated(restore);
    }
   ...
   */
  class ScaleSetter extends JDialog {
    Action scaleAction;
    ...
    private ScaleSetter() {
      super((Frame) drawingPanel.getTopLevelAncestor(), false);
      ...
      scaleAction = new AbstractAction() {
        public void actionPerformed(ActionEvent e) {
          ...
          switch(region) {
             case HORZ_MIN :
               max = drawingPanel.getXMax();
               break;
             case HORZ_MAX :
               min = drawingPanel.getXMin();
               break;
             case VERT_MIN :
               horzAxis = false;
               max = drawingPanel.getYMax();
               break;
             case VERT_MAX :
               horzAxis = false;
               min = drawingPanel.getYMin();
          }
          if(horzAxis) {
      ...
      scaleField = new ScientificField(5, 3) {
