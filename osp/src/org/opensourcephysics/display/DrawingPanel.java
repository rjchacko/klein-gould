...
public class DrawingPanel extends JPanel implements ActionListener, Renderable {
  ...
  public BufferedImage render(BufferedImage image) {
    ...
    imageRatio = image.getWidth()/(float) getWidth();
    if(osg!=null) {
      paintEverything(osg);
  ...
  public int getWidth() {
    return(int) (imageRatio*super.getWidth());
  }

  public int getHeight() {
    return(int) (imageRatio*super.getHeight());
  }
  ...
  public void snapshot() {
    ...
    frame.setTitle(DisplayRes.getString("DisplayPanel.Snapshot_menu_item")); //$NON-NLS-1$
    frame.pack();
  ...
  protected void scaleX(ArrayList<Drawable> tempList) {
    ...
    if(measurableFound) {
      if((xLeftMarginPercentage>0.0)||(xRightMarginPercentage>0.0)) {
  ...
  protected void scaleY(ArrayList<Drawable> tempList) {
    ...
    // do not change change values unless there is at least one measurable object.
    if(measurableFound) {
      if((yTopMarginPercentage>0.0)||(yBottomMarginPercentage>0.0)) {
  ...
  public class ZoomBox {
     ...
     */
    public Rectangle2D.Double reportZoom(int xpix, int ypix) {
      xstop = xpix;
      ystop = ypix;
  ...
  class OptionController extends MouseInputAdapter {
     ...
     */
    public void mouseReleased(MouseEvent e) {
      if(OSPRuntime.isPopupTrigger(e)&&(popupmenu!=null)&&popupmenu.isEnabled()) {
        if(isZoom()&&!zoomBox.isDragged()) {
