...
public class DataTable extends JTable implements ActionListener {
  ...
  protected HashMap<String, PrecisionRenderer> renderersByColumnName = new HashMap<String, PrecisionRenderer>();
  ...
  public void setFormatPattern(String columnName, String pattern) {
    if((pattern==null)||pattern.equals("")) { //$NON-NLS-1$
      renderersByColumnName.remove(columnName);
    } else {
      renderersByColumnName.put(columnName, new PrecisionRenderer(pattern));
    }
  }
  ...
  public String getFormatPattern(String columnName) {
    PrecisionRenderer r = renderersByColumnName.get(columnName);
    return (r==null) ? null : r.pattern;
  }
   ...
   */
  public NumberFormatDialog getFormatDialog(ArrayList<String> names, String selected) {
  ...
  public TableCellRenderer getCellRenderer(int row, int column) {
    ...
    try {
      ...
      Iterator<String> keys = renderersByColumnName.keySet().iterator();
      while(keys.hasNext()) {
        String columnName = keys.next();
        if(tableColumn.getHeaderValue().equals(columnName)) {
          return renderersByColumnName.get(columnName);
        }
      }
    } catch(Exception ex) {}
  ...
  protected static class DefaultDataTableModel implements DataTableModel {
    ...
    public String getColumnName(int columnIndex) {
      ...
      // code to strip underscores added by Doug Brown 2007-10-23
      // (another option: table header renderer to display subscripts
      // see DataToolTable for example implementation)
      return TeXParser.removeSubscripting(name);
    }
  ...
  public class NumberFormatDialog extends JDialog {
              ...
              tab+DisplayRes.getString("DataTable.NumberFormat.Help.Message3")+nl+                //$NON-NLS-1$
                tab+DisplayRes.getString("DataTable.NumberFormat.Help.Message4")+nl+              //$NON-NLS-1$
                  tab+DisplayRes.getString("DataTable.NumberFormat.Help.Message5")+nl+nl+         //$NON-NLS-1$
                    DisplayRes.getString("DataTable.NumberFormat.Help.Message6")+" \"12.34567\"", //$NON-NLS-1$ //$NON-NLS-2$
    ...
    private void showNumberFormatAndSample(String pattern) {
      if((pattern==null)||pattern.equals("")) { //$NON-NLS-1$
        ...
        Component c = renderer.getTableCellRendererComponent(DataTable.this, 12.34567, false, false, 0, 0);
    ...
    void setColumns(ArrayList<String> names, String selected) {
      columnNames = names;
      prevPatterns.clear();
      for(String name : columnNames) {
        prevPatterns.put(name, getFormatPattern(name));
      }
      showNumberFormatAndSample(prevPatterns.get(selected));
    }
