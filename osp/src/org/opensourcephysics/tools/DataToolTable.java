...
public class DataToolTable extends DataTable {
  ...
  TreeSet<Integer> selectedRows = new TreeSet<Integer>();         // selected model rows
  ...
  Color selectedBG, selectedFG, unselectedBG;
  ...
  public DataToolTable(DataToolTab tab) {
    ...
    selectionModel.addListSelectionListener(new ListSelectionListener() {
      public void valueChanged(ListSelectionEvent e) {
        ...
        for(int i = 0; i<rows.length; i++) {
          ...
          selectedRows.add(new Integer(modelRow));
        }
      }
    ...
    getTableHeader().addMouseListener(new MouseAdapter() {
      public void mouseClicked(MouseEvent e) {
        ...
        if(OSPRuntime.isPopupTrigger(e)) {
          ...
          cloneColumnsItem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
              ...
              for(int i = 0; i<colNames.size(); i++) {
                ...
                Dataset clone = dataToolTab.copy(data, null, false);
                ...
                ArrayList<DataColumn> loadedColumns = dataToolTab.loadData(clone);
          ...
          numberFormatItem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
              ...
              String selected = data.getYColumnName();
              ArrayList<String> cols = getSelectedColumnNames();
              if(!cols.contains(selected)) {
                cols.add(selected);
              }
              NumberFormatDialog dialog = getFormatDialog(cols, selected);
              java.awt.Dimension dim = dialog.getSize();
              java.awt.Point p = getLocationOnScreen();
              dialog.setLocation(p.x+mousePt.x-dim.width/2, p.y+mousePt.y);
        ...
        // double-click: select column
          ...
          // sort by row number
          decorator.sort(0);
          headerRenderer.sortCol = 0;
        }
        // single click label column: clear selection
        else if((col==labelCol)&&(headerRenderer.sortCol==col)) {
          clearSelection();
          leadCol = 1;
        }
        ...
        // single click: refresh header renderer and selected rows
        else {
          headerRenderer.sortCol = convertColumnIndexToModel(col);
          int[] rows = getSelectedModelRows();
    ...
    addMouseMotionListener(new MouseInputAdapter() {
      ...
      public void mouseDragged(MouseEvent e) {
        ...
        Dataset selected = getSelectedData();
        dataToolTab.curveFitter.setData(selected);
  ...
  protected HighlightableDataset getSelectedData() {
    ...
    dataToolTab.copy(workingData, selectedData, false);
    selectedData.clear();
  ...
  protected int getModelRow(int row) {
    int labelCol = convertColumnIndexToView(0);
    Integer n = (Integer) getValueAt(row, labelCol);
    return n.intValue();
  }
  ...
  protected int getViewRow(int row) {
    for(int i = 0; i<getRowCount(); i++) {
      ...
      Integer n = (Integer) getValueAt(i, col);
      if(row==n.intValue()) {
        return i;
      }
    }
  ...
  protected Dataset deleteColumn(String colName) {
    ...
    boolean sortColDeleted = headerRenderer.sortCol==index+1;
    if(sortColDeleted) {
      headerRenderer.sortCol = 0;
      decorator.sort(headerRenderer.sortCol);
    }
    // save selected rows and columns
    int[] rows = getSelectedModelRows();
    ArrayList<String> cols = getSelectedColumnNames();
  ...
  protected Collection<String> getPatternColumns() {
    return renderersByColumnName.keySet();
  }
  ...
  class HeaderRenderer implements TableCellRenderer {
    int sortCol;                                                 // model column currently sorted
    ...
    public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int col) {
      ...
      // set font: italics if undeletable, bold if sort column
      Dataset data = getDataset(name);
      Font font = comp.getFont();
      if(!dataToolTab.isDeletable(data)) {
        textLine.setFont((sortCol!=convertColumnIndexToModel(col)) ? font.deriveFont(Font.ITALIC) : font.deriveFont(Font.BOLD+Font.ITALIC));
      } else {
      ...
      textLine.setColor(selected ? selectedFG : comp.getForeground());
      textLine.setBackground(selected ? selectedBG : comp.getBackground());
      panel.setBackground(selected ? selectedBG : comp.getBackground());
      return panel;
    }
