  ...
  class PropsRenderer implements TableCellRenderer {
    ...
    public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int col) {
      ...
      if(value instanceof WorkingDataset) {
        ...
        panel.add(plot);
        panel.setToolTipText(ToolsRes.getString("DataToolPropsTable.Style.Tooltip"));     //$NON-NLS-1$
        ...
        panel.add(checkbox);
        if(row==markerRow) {
          panel.setToolTipText(ToolsRes.getString("DataToolPropsTable.Markers.Tooltip")); //$NON-NLS-1$
        } else {
          panel.setToolTipText(ToolsRes.getString("DataToolPropsTable.Lines.Tooltip"));   //$NON-NLS-1$
        }
      }
