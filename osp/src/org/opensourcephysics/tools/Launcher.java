...
public class Launcher {
   ...
   * @param pageNumber the tab to display
   * @return the LaunchNode
   */
  ...
  protected void createGUI(boolean splash) {
    ...
    backButton = new JButton(loadIcon(imageFile));
    ...
    forwardButton = new JButton(loadIcon(imageFile));
  ...
  protected void handleMousePressed(MouseEvent e, final LaunchPanel tab) {
    ...
    if(OSPRuntime.isPopupTrigger(e)) {
      ...
      if(node.getLaunchClass()!=null) {
        ...
        if(hasEJSModel(node)) {
          ...
          ejsItem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
              boolean quit = EjsTool.runEjs(node.getLaunchClass());                          // Paco added boolean quit =
              if(quit) {
                node.terminateAll();                                                         // Paco added this line
              }
            }
      ...
      refreshGUI();
    } else if((e.getClickCount()==2)&&(selectedNode.getLaunchClass()==null)&&(selectedNode.launchClassName!=null)&&!selectedNode.launchClassName.equals("")) { //$NON-NLS-1$
      // create jar list
