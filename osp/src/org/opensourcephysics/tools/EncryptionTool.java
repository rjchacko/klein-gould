...
public class EncryptionTool extends JFrame implements Tool {
  ...
  private String saveAs() {
    ...
    if(result==JFileChooser.APPROVE_OPTION) {
      ...
      if(file.exists()) {
        int selected = JOptionPane.showConfirmDialog(null, ToolsRes.getString("EncryptionTool.Dialog.ReplaceFile.Message")+" "+file.getName()+"?", //$NON-NLS-1$ //$NON-NLS-2$  //$NON-NLS-3$
