...
public class DatasetCurveFitter extends JPanel {
  ...
  static JButton newFitButton, deleteFitButton, cloneFitButton;
...
//    fitBuilderButton.setEnabled(true);
  ...
  protected void createGUI() {
    ...
    fitBuilderButton.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        autofitCheckBox.setSelected(false);
        deleteFitButton.setEnabled(!fitBuilder.getPanelNames().isEmpty());
        if (fit instanceof UserFunction 
        		&& fitBuilder.getPanelNames().contains(getFitName())) {
    ...
    // get fitBuilder and add existing user functions, if any
    getFitBuilder();
    ...
    if(fitBuilder==null) {
      ...
      newFitButton.addActionListener(new ActionListener() {
        public void actionPerformed(ActionEvent e) {
          ...
          FitFunctionPanel panel = new FitFunctionPanel(editor);
          fitBuilder.addPanel(name, panel);
        }
      });
      ...
      cloneFitButton.addActionListener(new ActionListener() {
        public void actionPerformed(ActionEvent e) {
          // inner popup menu listener class
        	...
        	for (DatasetCurveFitter fitter: fitBuilder.curveFitters) {
            ...
            public void actionPerformed(ActionEvent e) {
            	for (String name: fitnames) {
            		if (name.equals(e.getActionCommand())) {
            			...
            			UserFunction uf = createClone(f, name);
                  UserFunctionEditor editor = new UserFunctionEditor();
                  editor.setMainFunctions(new UserFunction[] {uf});
                  FitFunctionPanel panel = new FitFunctionPanel(editor);
                  fitBuilder.addPanel(uf.getName(), panel);    	
      ...
      fitBuilder = new FunctionTool(null, new JButton[] {newFitButton, cloneFitButton, deleteFitButton});
  ...
  private UserFunction createClone(KnownFunction f, String name) {
    String var = (dataset==null)? "x":                                         //$NON-NLS-1$
  	  TeXParser.removeSubscripting(dataset.getColumnName(0));
		f.getExpression(var);
