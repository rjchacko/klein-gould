  ...
  static private boolean doRunEjs(String _model, java.util.Set<String> _resources, Class<?> _ejsClass) {
    ...
    if(length>0) {
      for(String res : _resources) {
        extractList.add(res.startsWith(modelPath) ? "./"+res.substring(length) : res); //$NON-NLS-1$
      }
    } else {
    ...
    java.util.List<Object> finalList = ejsConfirmList(parentComponent, new java.awt.Dimension(400, 400), res.getString("EjsTool.ExtractingFiles"), res.getString("EjsTool.Message"), extractList, quitCheckBox); //$NON-NLS-1$ //$NON-NLS-2$
    if(finalList==null) {
      return false; // The user canceled
    }
