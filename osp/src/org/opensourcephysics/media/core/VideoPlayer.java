...
public class VideoPlayer extends JComponent implements PropertyChangeListener {
  ...
  public double getFrameTime(int frameNumber) {
    return clipControl.clip.getStartTime()-(clipControl.clip.getStartFrameNumber()-frameNumber)*clipControl.getMeanFrameDuration();
  }
      ...
      public void mousePressed(MouseEvent e) {
        ...
        for(int i = 0; i<readoutTypes.length; i++) {
          ...
          if(type.equals("step")) {                                                         //$NON-NLS-1$
            item = new JMenuItem(MediaRes.getString("VideoPlayer.Readout.MenuItem.Step"));  //$NON-NLS-1$ 
          } else if(type.equals("time")) {                                                  //$NON-NLS-1$
            item = new JMenuItem(MediaRes.getString("VideoPlayer.Readout.MenuItem.Time"));  //$NON-NLS-1$ 
          } else {
            item = new JMenuItem(MediaRes.getString("VideoPlayer.Readout.MenuItem.Frame")); //$NON-NLS-1$
          }
  ...
  private void updatePlayButton(boolean playing) {
    ...
    } else if((getVideoClip().getStepCount()<=clipControl.getStepNumber()+1)&&(getVideoClip().getStepCount()>1)) {
  ...
  private class VideoPlayerIcon implements Icon {
    ...
    VideoPlayerIcon(String name) {
      if(name.equals("play")) {             //$NON-NLS-1$
        createPlayIcon();
      } else if(name.equals("grayplay")) {  //$NON-NLS-1$
        createPlayIcon();
        setToGray();
      } else if(name.equals("pause")) {     //$NON-NLS-1$
        createPauseIcon();
      } else if(name.equals("reset")) {     //$NON-NLS-1$
        createResetIcon();
      } else if(name.equals("step")) {      //$NON-NLS-1$
        createStepIcon();
      } else if(name.equals("graystep")) {  //$NON-NLS-1$
        createStepIcon();
        setToGray();
      } else if(name.equals("back")) {      //$NON-NLS-1$
        createBackIcon();
      } else if(name.equals("grayback")) {  //$NON-NLS-1$
        createBackIcon();
        setToGray();
      } else if(name.equals("inspector")) { //$NON-NLS-1$
        createInspectorIcon();
      } else if(name.equals("loop")) {      //$NON-NLS-1$
        createLoopIcon();
      } else if(name.equals("noloop")) {    //$NON-NLS-1$
        createNoloopIcon();
      }
    }
