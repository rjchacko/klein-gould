...
public class VideoIO {
  ...
  static {
    qtFileFilter = new FileFilter() {
      public boolean accept(File f) {
        ...
        if(f.getAbsolutePath().indexOf("QTJava.zip")!=-1) {//$NON-NLS-1$
          return true; 
        }
        return false;
      }
      public String getDescription() {
        return "QTJava.zip";//$NON-NLS-1$
      }           
    };
  ...
  public static File open(File file, VideoPanel vidPanel) {
    ...
    if(videoFileFilter.accept(file)) {                                                             // load video
      ...
      if(video!=null) {
        ...
        JOptionPane.showMessageDialog(null, MediaRes.getString("VideoIO.Dialog.BadVideo.Message")+ //$NON-NLS-1$
      ...
      if(VideoPanel.class.isAssignableFrom(type)) {
        ...
        JOptionPane.showMessageDialog(null, "\""+file.getName()+"\" "+                             //$NON-NLS-1$ //$NON-NLS-2$
        ...
        JOptionPane.showMessageDialog(null, MediaRes.getString("VideoIO.Dialog.BadFile.Message")+  //$NON-NLS-1$
          file.getAbsolutePath());
      }
  ...
  public static File save(File file, VideoPanel vidPanel) {
    ...
    Video video = vidPanel.getVideo();
    if(video!=null) {
      video.setProperty("base", XML.getDirectoryPath(file.getAbsolutePath())); //$NON-NLS-1$
      if(video instanceof ImageVideo) {
