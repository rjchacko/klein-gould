...
import chris.ofcdamage.damageCG2D;
import chris.util.DirUtil;

public class grCGscalingApp extends Simulation{
	...
	private damageCG2D model;
	
	public static void main(String[] args) {
		new Control(new grCGscalingApp(), "Damage Parameters");
	}
	...
	public void load(Control c) {
		...
		params.add("Lattice Size", (int) 525);
		params.add("L'",(int) 25);
		params.add("\u03C4",(int) 100);
	...
	public void run() {
		...
		while(ccl < 25){	
			...
			while(Ndead < N){
				...
				if((dmt-1)%damageCG2D.dlength != 0){
					model.writeData(dmt);
					model.writeCGdata(dmt);
				}
