...
public class Car {
	...
	*/
	...
	public Vec3 wheelOffset(int i) {
		double sgn = (i == 0) ? 1 : -1;
		return new Vec3(sgn*wheel_offset_l, 0, 0).rotateZ(a);
	}
	
	public Vec3 wheelDir(int i) {
		double angle = (i == 0) ? a+steer : a;
		return new Vec3(1, 0, 0).rotateZ(angle);
	}
	
	public Vec3 wheelVel(int i) {
		return vel.plus(new Vec3(0,0,va).cross(wheelOffset(i)));
	}
	...
	void updateStep() {
		...
		Vec3 forces[] = new Vec3[] {new Vec3(0,0,0), new Vec3(0,0,0)};
		
		for (int i = 0; i < 2; i++) {
			Vec3 d = wheelDir(i);
			Vec3 v = wheelVel(i);
			Vec3 p = d.cross(new Vec3(0,0,1));
